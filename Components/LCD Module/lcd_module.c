 ...
 */
void Clear_Display(void) {     // Set PORTC to write (RS=0,RW=0)
    
    //  Set PORTC to write (RS=0,RW=0)
    PORTC = WRITE_INSTRUCTION;
    //  WRITE to PORTD
    TRISD = 0;
    //  Set PORTD to the "clear the display" instruction.
    PORTD = CLEAR_DISPLAY;
    //  Pulse Enable bit.
    Pulse_Enable_Bit();
...
bool Read_Busy_Flag_and_Address(void) 
{
    ...
    //  Set PORTC to read (RS=0,RW=1)
    PORTC = READ_BUSY_FLAG;
    //  READ from PORTD
    TRISD = 1;
    //  Pulse Enable bit.
    Pulse_Enable_Bit();
...
void Write_Data_to_CG_or_DD_RAM(unsigned char CG_or_DD_RAM) {
    ...
    //  Set PORTC to write (RS=1,RW=0)
    PORTC = WRITE_CG_OR_DD_RAM;
    //  WRITE to PORTD
    TRISD = 0;
    // data bits
    PORTD = CG_or_DD_RAM;
...
unsigned char Read_Data_from_CG_or_DD_RAM(void) {
    ...
    //  Set PORTC to read (RS=1,RW=1)
    PORTC = READ_CG_OR_DD_RAM;
    //  READ from PORTD 
    TRISD = 1;
    //  Pulse Enable bit.
    Pulse_Enable_Bit();
...
unsigned char Set_Instruction(unsigned char command, unsigned char instruction) 
{
    //  Set PORTC (port) to control
    PORTC = command;
    // control structure to implement different types of instruction
    if (command == WRITE_INSTRUCTION) 
    {   
        //PORTD = 0x00;
        //  WRITE to PORTD
        TRISD = 0;
        //  Set PORTD to instruction.
        PORTD = instruction;
        //  Pulse Enable bit.
        Pulse_Enable_Bit();
    ...
    else if(command == WRITE_CG_OR_DD_RAM)
    {
        //  WRITE to PORTD
        TRISD = 0;
        //  Set PORTD to instruction.
    ...
    else if(command == READ_BUSY_FLAG)
    {
        //  READ from PORTD
        TRISD = 1;
        //  Pulse Enable bit.
